...
def filter_samples(samples, threshold=0.05):
    ...
    filtered_label = torch.masked_select(samples['label'], mask.bool())
    filtered_gt = torch.masked_select(samples['gt'], mask.bool()) \
                     if samples['gt'] is not None else None

    filtered_samples = {}
    filtered_samples['data'] = filtered_data
    filtered_samples['label'] = filtered_label
    # filtered_samples['label'] = filtered_gt
...
def split_samples_classwise(samples, num_classes):
    ...
    for c in range(num_classes):
        ...
        label_c = torch.masked_select(label, mask.bool())
        gt_c = torch.masked_select(gt, mask.bool()) if gt is not None else None
        samples_c = {}
        samples_c['data'] = data_c
        samples_c['label'] = label_c
        # samples_c['label'] = gt_c
        samples_c['gt'] = gt_c
...
def adjust_learning_fix(lr, optimizer):
    for param_group in optimizer.param_groups:
