...
class DANSolver(BaseSolver):
    def __init__(self, net, dataloader, bn_domain_map={}, resume=None, **kwargs):
        super(DANSolver, self).__init__(net, dataloader, \
    ...
    def update_network(self, filtered_classes):
        ...
        while not stop:
            ...
            if len(filtered_classes) > 0:
                ...
                p = float(self.iters_per_loop * self.loop +  update_iters) / (self.opt.TRAIN.MAX_LOOP * self.iters_per_loop)
                ...
                combined_image = torch.cat((source_cls_concat, target_cls_concat), 0)
                combined_feature = self.net(combined_image)['feat']
                domain_pred = self.discriminator(combined_feature, alpha)

                domain_source_labels = torch.zeros(source_cls_concat.shape[0]).type(torch.LongTensor)
                domain_target_labels = torch.ones(target_cls_concat.shape[0]).type(torch.LongTensor)
            ...
            if self.opt.TRAIN.LOGGING and (update_iters+1) % \
                ...
                cur_loss = {'ce_loss': ce_loss_iter, 'domain_loss': domain_loss,
			'total_loss': loss}
                self.writer.add_scalar("ce_loss", ce_loss_iter, self.iters)
