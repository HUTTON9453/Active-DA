...
class MMESolver(BaseSolver):
    def __init__(self, net, dataloader, X, Y, bn_domain_map={}, resume=None, \
        ...
        super(MMESolver, self).__init__(net, dataloader, X, Y, \
        ...
        self.writer = SummaryWriter(f'./experiments/runs/MME/{self.opt.EXP_NAME}')
        #self.q_idxs = []
    ...
    def build_active_learning_dataloader(self, index):
        #if index==0:
        #    self.q_idxs = self.query(self.opt.FINETUNE.NUM_QUERY)
        #q_idxs = self.q_idxs[int(self.opt.FINETUNE.NUM_QUERY/5*index):int(self.opt.FINETUNE.NUM_QUERY/5*(index+1))]
    ...
    def solve(self):
        ...
        for i in range(self.opt.MME.FIRST_MAX_EPOCH):
            self.update_network()
            self.loop += 1
        ...
        for j in range(self.opt.FINETUNE.NUM_ROUND):
            ...
            for i in range(self.opt.MME.MAX_EPOCH):
                self.update_network()
                self.loop += 1
            with torch.no_grad():
    ...
    def update_network(self):
        ...
        while not stop:
            ...
            source_ce_loss = self.CELoss(source_preds, source_gt)
            if self.train_data[self.target_name+'_labeled']['loader'] is not None:
                target_preds = self.net(target_label_data)['logits']
                target_ce_loss = self.CELoss(target_preds, target_label_gt)
            else:
                target_ce_loss = 0
            loss = self.opt.MME.lambda_s * source_ce_loss + self.opt.MME.lambda_t * target_ce_loss
            loss.backward(retain_graph=True)
            # update the network
            self.optimizer.step()
            #self.net.zero_grad()
            self.optimizer.zero_grad()
            loss_t = self.adentropy(target_data, self.opt.MME.lambda_h)
            loss_t.backward()
            # update the network
            self.optimizer.step()
            ...
            if self.opt.TRAIN.LOGGING and (update_iters+1) % \
                ...
                cur_loss = {'source_ce_loss': source_ce_loss,'target_ce_loss': target_ce_loss,"loss_t": -loss_t, 'loss':loss}
                self.writer.add_scalar("source_ce_loss", source_ce_loss, self.iters)
                self.writer.add_scalar("target_ce_loss", target_ce_loss, self.iters)
                self.writer.add_scalar("loss_t", -loss_t, self.iters)
