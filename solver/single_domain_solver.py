...
class SingleDomainSolver(BaseSolver):
    def __init__(self, net, dataloader, X, Y, resume=None, **kwargs):
        super(SingleDomainSolver, self).__init__(net, dataloader, X, Y, \
    ...
    def solve(self):
        ...
        while True:
            if self.loop > self.opt.TRAIN.FIRST_MAX_LOOP: break
            self.update_network()
            self.loop += 1
